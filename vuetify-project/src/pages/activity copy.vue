...
<v-col cols="12" md="8">
  <v-card style="width: 100%; height: 100%;" color="primary">
    <v-card-title class="d-flex">
      ...
      <v-spacer></v-spacer>
      <v-btn color="secondary" @click="openDialog(null)">創建</v-btn>
    </v-card-title>
      ...
      <v-card-item>
        <v-row>
          <v-col cols="6">
                ...
                <v-img :src="headers.event.image" height="200px" contain></v-img>
                <v-card-title>{{ headers.event.name }}</v-card-title>
                <v-card-subtitle>價格: ${{ headers.event.price }}</v-card-subtitle>
                <v-card-subtitle>主持人: {{ headers.event.host }}</v-card-subtitle>
...
const headers = [
    { title: 'ID', key: '_id'},
  { title: ('event.image'), key: 'image' },
  { title: ('event.safe'), key: 'safe' },
  { title: ('event.enablePerson'), key: 'enablePerson' },
    { title: ('event.name'), key: 'name'},
    { title: ('event.Random'), key: 'Random'},
  { title: ('event.description'), key: 'description' },
  { title: ('event.host'), key: 'host' },
    { title: ('event.price'), key: 'price'},
    { title: ('adminEvent.edit'), key: 'edit' },
  ]
</script>
